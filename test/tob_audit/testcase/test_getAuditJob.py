...
from test.tob_audit.data.auditParams import getAuditJob_params
import requests
import time
...
tob_audit_port = '8082'
tob_audit_interface = '/audit/getAuditJob'
...
def test_getAuditJob():
    ...
    #参数变量
    params = {
        'orderId' : getAuditJob_params.orderId,
        'sysUserID' : getAuditJob_params.sysUserId,
        'auditStatus' : getAuditJob_params.auditStatus,
        'jobFrom' : getAuditJob_params.jobFrom,
        'pageNumber' : getAuditJob_params.pageNumber,
        'pageSize' : getAuditJob_params.pageSize}

    audit_getAudit_api = 'http://' + tob_audit_host + ':' + tob_audit_port + tob_audit_interface

  #发起请求
    r = requests.post(url=audit_getAudit_api,json=params,headers = headers)
